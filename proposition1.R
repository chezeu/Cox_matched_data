...
#<ek <- 0.03 # erreur
p2 <- 0.7# proba de la loi binomiale
...
equa_estimat3 <- function(beta, data_naive, XB, p,Q) {
  m= nrow(XB)
  n = nrow(data_naive)
  ...
  for (j in 1:n) {
    ...
    qj <- qj[L1]
    eX <- eXbeta[-j]
    produit3[j] <-  ((1-qj[j])/ (m-1))^(m-1) *(prod(eX))
  }  
  produit3
  tilde_f2 <- (ezbeta /qj[j]) * (1/produit3)
  ...
  for (k in 1:n) {
    qk <- Q[k,]
    eX <- eXbeta[-k]
    produit5[k]<- ((1-qk[k])/(m-1))^(m-1) *prod(eX)
    som5[k,]<-  colSums(X[-k,]) * eXbeta[k]
  }  
  produit5  
  som5
  tilde_g2 <- (1/qk[k])*(zezbeta/produit5) - som5
  ...
  for(i in 1:n){ 
    qi <- Q[i,]
    Xj <- X[-i,]
    tilde_x[i,] <- (1/qi[i])*Z[i,]  - (1/qi[i])*colSums(Xj)*( (1-qi[i])/(m-1) )
  }
  tilde_x
...
coxph_equa_estimat2 <- function(data_naive, XB, p, Q,maxiter = 20){
  f2 <- function(x){
    equa_estimat3 (beta=x, data_naive=data_naive, XB = XB, p = p,Q=Q )
  }
...
colnames(base_result)[1] <- "x1_naive"
colnames(base_result)[2] <- "x2_naive"
colnames(base_result)[3] <- "x1_estimat"
colnames(base_result)[4] <- "x2_estimat"
base_result <- data.frame(base_result)
w <- which(base_result$conver_estimat!=0)
