...
equa_estimat <- function(beta, data_naive, XB, p,Q) {
  ...
  produit5<- matrix(0,nrow=n, ncol = p)
  for (j in 1:n) {
    qj <- Q[j,]
    qjx <- (qj*eXbeta)
    L1 <- which(qj!=0)
  ...
  for(i in 1:n){ 
    ...
    num1 <-colSums( tilde_g2[risq,])
    denum1 <- sum(tilde_f2[risq])  
    
    num_naive<-colSums( zezbeta[risq,])
    denum_naive<- sum(ezbeta[risq])
    
    s2<- s2+ event[i]*( (Z[i,]-tilde_x[i,]) -  (num_naive/denum_naive - num1/denum1 ))
  }
  s2
  return(H=s2)  
}
