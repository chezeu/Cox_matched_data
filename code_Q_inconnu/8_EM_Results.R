...
beta= c(0.5,-0.5)
  ...
  for (i in 1:nrow(scenarios)) {
    ...
    load(paste0("C:/Users/fchezeut/Documents/GitHub/Cox_matched_data/code_Q_connu/Results/",
                "nsim=",nsim,"_m=",m, "_prob1=", C[1], ".Rdata"))
    ...
    beta_estimate_Nd[i,] = rbind( mean(results_sample$coef_estimate_s_Nd1 ), mean(results_sample$coef_estimate_s_Nd2 ))
    ...
    bias_estimate_Nd[i,]  = rbind( abs(mean(results_sample$coef_estimate_s_Nd1) -  beta[1]),
                                abs(mean(results_sample$coef_estimate_s_Nd2) -  beta[2] ))
    # number of convergences
    ...
    L_estimate_Nd[i] = length(which(results_sample$converge_estimate_Nd==0))
    ...
    Sd_estimate_Nd[i,] = rbind( sd(results_sample$coef_estimate_s_Nd1),sd(results_sample$coef_estimate_s_Nd2))
    ...
    rmse_estimate_Nd[i,] = rbind( sqrt( mean((results_sample$coef_estimate_s_Nd1 -  beta[1])^2)),
                               sqrt( mean((results_sample$coef_estimate_s_Nd2 -  beta[2])^2)))
    
    Results_scenario = cbind( c(0,L_naive[i],L_w_sum1[i],L_w_sum2[i], L_estimate[i],L_estimate_Nd[i]),
                              c( beta_true[i,1],beta_naive[i,1],beta_w_sum1[i,1],beta_w_sum2[i,1],beta_estimate[i,1],beta_estimate_Nd[i,1]),
                              c(bias_true[i,1],  bias_naive[i,1],bias_w_sum1[i,1],bias_w_sum2[i,1],bias_estimate[i,1],bias_estimate_Nd[i,1]),
                              c(Sd_true[i,1], Sd_naive[i,1],Sd_sum1[i,1],Sd_sum2[i,1],Sd_estimate[i,1],Sd_estimate_Nd[i,1]),
                              c(rmse_true[i,1],rmse_naive[i,1], rmse_sum1[i,1], rmse_sum1[i,1],rmse_estimate[i,1],rmse_estimate_Nd[i,1]),
                              c( beta_true[i,2],beta_naive[i,2],beta_w_sum1[i,2],beta_w_sum2[i,2],beta_estimate[i,2],beta_estimate_Nd[i,2]),
                              c(bias_true[i,2],  bias_naive[i,2],bias_w_sum1[i,2],bias_w_sum2[i,2],bias_estimate[i,2],bias_estimate_Nd[i,2]),
                              c(Sd_true[i,2], Sd_naive[i,2],Sd_sum1[i,2],Sd_sum2[i,2],Sd_estimate[i,2],Sd_estimate_Nd[i,2]),
                              c(rmse_true[i,2],rmse_naive[i,2], rmse_sum1[i,2], rmse_sum1[i,2],rmse_estimate[i,2],rmse_estimate_Nd[i,2] ) )
    
    colnames( Results_scenario) = c( "Fails","beta.1", "Bias.1", "Sd.1", "Rmse.1","beta.2", "Bias.2", "Sd.2", "Rmse.2")
    
    rownames( Results_scenario) = c( "Theorical", " Naive", "W-sum1", "W-sum2", "EM", "EM_Nd" )
    
    print(c("m=",m, "prob_1=", C[1] ))
