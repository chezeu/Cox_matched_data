...
generate_link_data <- function(m, n, K, prevalence, error, min_prev = 0.01){
  ...
  databaseB[,1:2] <- as.matrix(surv_data[,1:2])
  databaseB [,3: ncol(databaseB) ]<-as.matrix( datB)
  databaseB <- data.frame(databaseB)
  colnames(databaseB)=c("Time","delta",paste("R", 1:K, sep = ""),"id") 
  
  databaseA <- matrix(0,n,(p+K+1))
  databaseA[,1:2] <- as.matrix(surv_data[idBA,3:(p+2)])
  databaseA [,3: ncol(databaseA) ]<-as.matrix( datA)
  databaseA <- data.frame(databaseA)
  colnames(databaseA)=c("x1","x2",paste("R", 1:K, sep = ""),"id") 
  
  datA <- databaseA[,-c(1,2)]
  datB <-  databaseB[,-c(1,2)]
  
  return(list(databaseA=databaseA, databaseB = databaseB, prev = prevalence,datA=datA,datB=datB))
}
...
EM_binary <- function(comp_mat, datA, datB, K, e = 0.02, tol = 1e-5, maxits = 100){
...
EM <- EM_binary (comp_mat, datA, datB, K, e = 0.02, tol = 1e-5, maxits = 100)
g <- EM$g
convr <- EM$converge
...
generate_naive_data <- function(comp_mat.1,databaseB, databaseA){
  ...
  data_naive$Time <- covar_naive_B$Time
  data_naive$delta <-  covar_naive_B$delta
  
  
  return(list(data_naive=data_naive, Naive_max=Naive_max,   XB=  XB))
}
...
######################
Matrix_function <-function(n,m,C,beta,databaseB, databaseA, estimate=TRUE, comp_mat){
  
 surv_data <- Generate_surv_data(m,n,beta,p)
  Time <- surv_data[,1:2]
  TimeB<- as.matrix(Time, ncol = p) #Only covariates
 ...
 # data_A <- surv_data[1:n,1:2] #Survival time and censored indicator
  ...
  if(estimate==TRUE){ 
    ...
    Lvec
    Time_naive <-Lvec %*% TimeB
    TimeZ<- Time_naive
    colnames(TimeZ)<- c("TimeZ","deltaZ")
    # Naive data
    data_naive <- cbind( datA, TimeZ)
    #trueLink_id <- diag(Lvec)
    
  }else{
    
    C=c(0,0,0)
    EM <- EM_binary (comp_mat, datA, datB, K, e = 0.03, tol = 1e-5, maxits = 200)
    Q <- EM$Q
    
    
  }
  return(list( datA= datA,TimeB=TimeB,Q=Q,data_naive =data_naive ))
}

C <- c(0.8,0.2,0.1)
func <- Matrix_function (n,m,C,beta,databaseB, databaseA, estimate=TRUE,comp_mat)
Q <- func$Q
TimeB <- func$TimeB
data_naive <- func$data_naive
######################################################### cox model ###########
...
equa_estimat <- function(beta, data_naive, TimeB, p,Q) {
  
  X <-as.matrix( data_naive[,1:2]) 
 
  TimeZ <- as.matrix(data_naive[,3])
  eventZ<- data_naive[,(p+2)]
  Time <- as.matrix(TimeB[,1])
  ...
  #######les sommes pour tilde_Time
  
  som0 <- matrix(0,nrow=n, ncol = 1)
  for(i in 1:n){ 
    qi <- Q[i,]
    qix <- (qi*Time)[-i]
    som0[i] <- sum(qix)
  ...
  som0
  diagonal <- diag(Q)
  tilde_Time <- (TimeZ/diag(Q))-som0/diag(Q)  #on observe des temps negatifs 
  
  
  dat1 <- cbind( tilde_Time,diagonal, X)[which(eventZ==1),]
  ...
  for (i in 1:nrow(dat1)) {
    ...
    risk <- which(ts<= tilde_Time & eventZ==1)
    nrisk <- length(risk)
    if(nrisk==1){
      t2R <-  eXbeta[risk] 
      t3R <- matrix( XeXbeta, ncol = p)[risk,] 
      
    }else if(nrisk!=1){
      
      t2R <- sum( eXbeta[risk] )
      t3R <- colSums(matrix(  XeXbeta, ncol = p)[risk,] ) }
...
equa_naive <- function(beta, data_naive, TimeB, p) {
  m= nrow(TimeB)
  n = nrow(data_naive)
...
#naive_H <- equa_naive (beta, data_naive, TimeB, p) 
##################################################################
################### resoudre les equations 

coxph_equa_naive <- function(data_naive, TimeB, p,maxiter = 100){
  f <- function(x){
    equa_naive (beta=x, data_naive=data_naive, TimeB = TimeB, p = p )
  }
...
coxph_equa_estimat2 <- function(data_naive, TimeB, p,Q,maxiter = 100){
  f2 <- function(x){
    equa_estimat (beta=x, data_naive=data_naive, TimeB = TimeB, p = p, Q=Q )
  }
...
sol <- coxph_equa_estimat2 (data_naive, TimeB, p,Q,maxiter = 100)
sol
################################################
...
doOne2d_equat_estimat<- function(n,Q,databaseB, databaseA){
  # Fixed parameters
  m = 2*n
  #rho = 0.75
  p = 2
  ...
  fit_naive <- coxph(Surv(TimeZ,deltaZ)~.,data = data_naive)
  ...
  fit_estimat <- coxph_equa_estimat2(data_naive, TimeB, p, Q,maxiter = 100)
  coef_estimat <- fit_estimat$coef
  converge_estimat <- fit_estimat$converge
  
  fit_naive2 <- coxph_equa_naive(data_naive, TimeB, p,maxiter = 100)
  coef_naive2 <- fit_naive2$coef
  converge_naive2 <- fit_naive2$converge
  
  return(list (  coef_naive= coef_naive , coef_naive2=coef_naive2, converge_naive2=converge_naive2,
...
test_estimat <- function(nsim, Q){
  for (i in 1:nsim) {
    g <- doOne2d_equat_estimat (n,Q,databaseB, databaseA)
...
result_estimat <- test_estimat(100,Q)
