...
source("3_EM_record_linkage.R")
source("4_EM_naive_method.R")
source("5_EM_method_EM.R")
source("6_EM_scenarios.R")
...
estimates_survival<- function(nsim,nA,nB,p,beta,K,prevalence){
  ...
  for (i in 1:nsim){
    #prevalence= rep(c(0.5, 0.5,0.5,0.5,0.5,0.5,0.5,0.5,0.5),K/9)
    #data generation
    ...
    linked = EM_binary ( comp_mat,datA, datB, K, e =error, tol = 1e-5, maxits = 1000)
    Q=linked$Q
    apply(Q,1,sum)
...
for (i in (1:nrow(scenarios))){
  ...
  filename = paste0("H:/code_record/Results/","nsim=",nsim,
                    "_nA=",nA, "_K=",K,".Rdata")
