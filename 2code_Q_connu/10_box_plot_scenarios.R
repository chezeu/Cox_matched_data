...
beta0_3 = ggplot(boxplot_beta0,aes(x=nA,y= beta0.3,fill=Estimator))+
  ...
  geom_hline(yintercept=beta[3],lty=1,col="orange")+
  ggtitle(titlename)
beta0_3+ scale_x_discrete(limits=c(order[1],order[2],order[3]))
...
for (i in 1:length(nA_hist)) {
  ...
  hist(beta_hist$beta0.3, xlab= quote(hat(beta)[3]),main = paste("nA =", n,",","prob=",unique(data_histogram$Prob_1)))
}
