...
C<-c(0.6,0.4,0.2)## posterior probability values
...
equa_estimat4 <- function(beta, data_naive, XB, p,Q) {
  m= nrow(XB)
  n = nrow(data_naive)
  ...
  for (j in 1:n) {
    ...
    a <- which(exk2==eXk) 
    xk2 <- exk2[-a]
    produit3[j] <- prod(qj)-prod((1/exk2))
  }  
  produit3
  tilde_f2 <- ezbeta- prod(eXbeta[L1]) *produit3
  ...
  for (k in 1:n) {
    ...
    a <- which(ex==eX2)
    eX2 <-  eX2[-a]
   # I <-matrix(0,nrow = m,ncol = p)
  # I[1,] <- c(1,1)
    produit5[k]<- (X[k])*prod(1/eX2)
    som5[k,]<-  colSums(xj) * prod(qk)
    
  }  
  produit5  
  som5
  tilde_g2 <- zezbeta - prod(eXbeta[L2])*(som5- produit5 ) 
...
coxph_equa_estimat2 <- function(data_naive, XB, p, Q,maxiter = 50){
  f2 <- function(x){
    equa_estimat4 (beta=x, data_naive=data_naive, XB = XB, p = p,Q=Q )
  }
  init2 <- rep(0,p)
...
################## monte carlo ##############################################################
R <- 100 ## nombre de generation de données
