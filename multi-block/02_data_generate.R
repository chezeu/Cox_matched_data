...
########################################################
#generation of the bigger data base

Generate_data <- function(K,nA,nB,prevalence, min_prev = 0.01){
  #prevalence = rep(0.5, K)
  ...
  Time      = pmin(Tt,c) # true times 
  delta     = as.numeric(Tt<=c)
  surv_data = data.frame(Time,delta,X) # true data
