...
estimates_survival<- function(nsim,nA,nB,C,p,beta){
  ...
  coef_EM = matrix(0,nrow = nsim, ncol = p)
  ...
  for (i in 1:nsim){
    ...
    #method with the maximum of 2 max proba
    ...
    coef_EM[i,] = fit_estimate$beta0
    converge_estimate[i] = as.numeric(fit_estimate$converge)
    
  }
  
  return(list( coef_true_s1 = coef_true_s[,1], coef_true_s2 = coef_true_s[,2],coef_true_s3 = coef_true_s[,3],coef_true_s4 = coef_true_s[,4],
           coef_naive_s1 = coef_naive_s[,1], coef_naive_s2 = coef_naive_s[,2], coef_naive_s3 = coef_naive_s[,3], coef_naive_s4 = coef_naive_s[,4],
           coef_w_sum1_s1 = coef_w_sum1_s[,1], coef_w_sum1_s2 = coef_w_sum1_s[,2],  coef_w_sum1_s3 = coef_w_sum1_s[,3], coef_w_sum1_s4 = coef_w_sum1_s[,4],
           coef_w_sum2_s1 = coef_w_sum2_s[,1], coef_w_sum2_s2 = coef_w_sum2_s[,2], coef_w_sum2_s3 = coef_w_sum2_s[,3], coef_w_sum2_s4 = coef_w_sum2_s[,4],  
          coef_EM1 = coef_EM[,1], coef_EM2 = coef_EM[,2],coef_EM3 = coef_EM[,3], coef_EM4= coef_EM[,4], 
          converge_naive = converge_naive,converge_w_sum1= converge_w_sum1,converge_w_sum2 = converge_w_sum2, converge_estimate = converge_estimate))} 
######################################
...
for (i in (1:nrow(scenarios))){
  ...
  results_sample = estimates_survival(nsim,nA,nB,C,p,beta) #monte carlos
