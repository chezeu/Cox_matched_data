...
#scenarios_boxplot=scenarios[scenarios$nA==700,] 
...
for (i in (1:nrow(scenarios_boxplot ))){
  ...
  Prob4 = scenarios_boxplot[i,7]
  Prob5 = scenarios_boxplot[i,8]
  
  Prob_sample = paste0("(",Prob1,",",Prob2,",",Prob3,",",Prob4,",",Prob5,")") 
  ...
  boxplot_beta0_estimate = cbind(rep(5,times=nsim),
                                 rep(nA,times=nsim),
                                 rep(  Prob_sample , times=nsim ),
                                 results_sample$coef_EM1,
                                 results_sample$coef_EM2,
                                 results_sample$coef_EM3,
                                 results_sample$coef_EM4)
...
beta0_4 = ggplot(boxplot_beta0,aes(x=Q,y= beta0.4,fill=Estimator))+
  ...
  geom_hline(yintercept=beta[4],lty=1,col="orange")+
  ggtitle(titlename)
beta0_4
######################## ####################
...
source("37_scenarios.R")
 ...
 scenarios_boxplot=scenarios[scenarios$Prob_1==0.2,] 
...
titlename = paste0( "Q=", "(",unique(scenarios_boxplot[,4]), ",",
    ...
    ",",unique(scenarios_boxplot[,7]),",",unique(scenarios_boxplot[,8]), ")" )
